...
export const sendMessage = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (args.body.startsWith("/google")) {
      // Get the string after the first space
      const topic = args.body.slice(args.body.indexOf(" ") + 1);
      await ctx.scheduler.runAfter(0, internal.chat.googlesummary, {
        topic,
      });
    }
...
export const googlesummary = internalAction({
  args: { topic: v.string() },
  handler: async (ctx, args) => {
    const response = await fetch(
      `https://api.duckduckgo.com/?q=${encodeURIComponent(args.topic)}&format=json`
    );

    const data = await response.json();
    const summary =
      data.AbstractText || "No summary found for this topic.";

    await ctx.scheduler.runAfter(0, api.chat.sendMessage, {
        user: "Google",
        body: `Here’s your search: https://www.google.com/search?q=${encodeURIComponent(args.topic)}`,
    });
